...
// apps/playground-app-ai/src/verify/verification-ollama.ts
var PROVIDER = "ollama";
...
var apiKey = (args2.apiKey ?? process.env.OLLAMA_API_KEY)?.trim();
var host = (args2.baseUrl ?? process.env.OLLAMA_HOST)?.trim() || "http://127.0.0.1:11434";
var chatModels = args2.models ?? splitCsv(process.env.OLLAMA_CHAT_MODELS) ?? [process.env.OLLAMA_CHAT_MODEL?.trim() || "gemma4:12b"];
var embedModel = args2.embedModel ?? process.env.OLLAMA_EMBED_MODEL?.trim() ?? "embeddinggemma:latest";
if (chatModels.length === 0) {
  console.error("No chat models configured for Ollama. Use --models or OLLAMA_CHAT_MODELS.");
  process.exit(1);
}
var imagePath = args2.image ?? OLLAMA_DEFAULT_IMAGE_PATH;
var image = await createLocalImagePart(imagePath).catch((error) => {
  if (args2.image) {
    throw error;
  }
  console.warn(`Default image not found at ${imagePath}: ${error instanceof Error ? error.message : String(error)}. Falling back to a generated image.`);
  return;
});
...
await runProviderVerification({
  ...
  chatModels,
  embedModel,
  supportsEmbedding: !args2.skipEmbedding && Boolean(embedModel),
  defaultModelCapabilities: {
    inputModalities: ["text", "image"],
    reasoning: true,
    toolCall: true
  },
  skipVision: args2.skipVision,
