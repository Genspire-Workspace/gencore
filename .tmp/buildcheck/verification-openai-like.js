...
// apps/playground-app-ai/src/verify/verification-openai-like.ts
...
var apiKey = (args2.apiKey ?? process.env.DEEPSEEK_API_KEY)?.trim();
if (!apiKey) {
  console.error("DEEPSEEK_API_KEY is not set. Set it in .env or pass --api-key.");
  process.exit(1);
}
var baseURL = args2.baseUrl ?? process.env.DEEPSEEK_BASE_URL?.trim() ?? "https://api.deepseek.com/v1";
var chatModels = args2.models ?? splitCsv(process.env.DEEPSEEK_CHAT_MODELS) ?? [process.env.DEEPSEEK_CHAT_MODEL?.trim() || "deepseek-v4-flash"];
var embedModel = args2.embedModel ?? process.env.DEEPSEEK_EMBED_MODEL?.trim();
